  ...
  return (
    <>
      <div>
        ...
        <div className="text-sm text-slate-400">Audit trail of closed trades (paper)</div>
      </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {((q.data?.trades as any[]) ?? []).map((t, idx) => (
                      <tr key={t.id ?? idx} className="border-t border-slate-800">
                        <td className="py-2">
                          {t.instrument_key ? <InstrumentLabel query={String(t.instrument_key)} /> : <span className="text-slate-400">-</span>}
                        </td>
                        <td className="text-slate-200">{t.side ?? t.action ?? '-'}</td>
                        <td className="text-slate-200">{t.quantity ?? t.qty ?? '-'}</td>
                        <td className="text-slate-200">{t.entry_price ?? t.entry ?? '-'}</td>
                        <td className="text-slate-200">{t.exit_price ?? t.exit ?? '-'}</td>
                        <td className="text-slate-200">{t.pnl ?? t.p_and_l ?? '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
