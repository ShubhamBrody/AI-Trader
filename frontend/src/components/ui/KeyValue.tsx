...
export function KeyValue({ label, value }: { label: string; value: any }) {
  ...
  return (
    <div className="grid grid-cols-2 items-center gap-3 rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2">
      <div className="min-w-0 truncate text-xs text-slate-400" title={label}>
        {label}
      </div>
      <div
        className={
          isPlaceholder
            ...
            : 'min-w-0 truncate text-right text-sm text-slate-100'
        }
        title={title}
      >
        {display}
