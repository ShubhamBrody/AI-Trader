  ...
  return (
    <div ref={boxRef} className="relative">
      ...
      {open && value.trim().length > 0 ? (
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-xl border border-slate-800 bg-slate-950/95 shadow-xl">
          {suggest.isLoading ? (
            ...
            <div className="max-h-[260px] overflow-auto">
              {items.map((it) => (
                <button
                  ...
                  className="flex w-full items-start justify-between gap-3 border-t border-slate-800 px-3 py-2 text-left hover:bg-slate-900/60"
                >
                  <div>
                    <div className="text-sm font-semibold text-slate-100">{it.canonical_symbol}</div>
