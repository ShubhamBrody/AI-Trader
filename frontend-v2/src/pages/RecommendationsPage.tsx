...
export function RecommendationsPage() {
  ...
  const [mode, setMode] = useState<'classic' | 'statistical'>(() => {
    try {
      const v = window.localStorage.getItem('recs_mode');
      return v === 'statistical' ? 'statistical' : 'classic';
    } catch {
      return 'classic';
    }
  });
  ...
  const queryUrl = useMemo(() => {
    if (mode === 'statistical') {
      return '/api/recommendations/statistical/top?n=10&min_confidence=0.6&max_risk=0.7&universe_limit=50&interval=5m&lookback_days=30&horizon_steps=12&max_patterns=3';
    }
    return '/api/recommendations/top?n=10&min_confidence=0.6&max_risk=0.7';
  }, [mode]);

  const q = useQuery({
    queryKey: ['recommendations', mode, queryUrl],
    queryFn: () => apiGet<any>(queryUrl),
    retry: 0,
  });

  const items = (q.data?.recommendations as any[]) ?? [];
  const meta = q.data?.meta ?? {};
  const isStale = mode === 'classic' ? Boolean(meta?.is_stale) : false;
  const servedFromBackup = mode === 'classic' ? Boolean(meta?.served_from_backup) : false;
  const cacheDay = mode === 'classic' && typeof meta?.cache_trading_day === 'string' ? meta.cache_trading_day : null;
  const createdAt = mode === 'classic' && typeof meta?.created_at_utc === 'string' ? meta.created_at_utc : null;
      ...
      try {
        ...
        if (msg?.channel === 'recommendations' && (msg?.type === 'refreshed' || msg?.type === 'refresh_failed')) {
          if (msg?.type === 'refreshed') {
            ...
            qc.invalidateQueries({ queryKey: ['recommendations', 'classic'] });
          } else {
            setRefreshJobId(null);
            setRefreshProgress(null);
  ...
  return (
    <>
      <div>
        ...
        <div className="text-sm text-slate-600 dark:text-slate-400">Top opportunities for the day</div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => {
              setMode('classic');
              try {
                window.localStorage.setItem('recs_mode', 'classic');
              } catch {
                // ignore
              }
              setRefreshError(null);
          ...
          <Button
            size="sm"
            variant={mode === 'statistical' ? 'primary' : 'secondary'}
            onClick={() => {
              setMode('statistical');
              try {
                window.localStorage.setItem('recs_mode', 'statistical');
              } catch {
                // ignore
              }
              setRefreshError(null);
        ...
        {mode === 'classic' ? (
          ...
          <Badge tone="neutral">Computed on demand</Badge>
        )}
        {refreshJobId ? (
          <div className="flex items-center gap-2">
          ...
          {q.isLoading ? (
            ...
            <div className="overflow-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((r, idx) => {
                    const action = String(r.predicted_action ?? r.signal ?? 'HOLD');
                    const scoreVal = r.score ?? r.expected_return ?? null;
                    const confVal = r.confidence ?? null;
                    const reasonsRaw = (r.reasons ?? r.reason) as any;
                    ...
                    return (
                      <tr key={String(r.instrument_key ?? r.symbol ?? idx)} className="border-t border-slate-200 dark:border-slate-800">
                        <td className="py-2">
                          <div className="font-medium text-slate-900 dark:text-slate-100">{String(r.symbol ?? '—')}</div>
                          <div className="text-xs text-slate-500">{String(r.instrument_key ?? '')}</div>
                        </td>
                        <td className="py-2">
                          <Badge tone={action === 'BUY' ? 'good' : action === 'SELL' ? 'bad' : 'neutral'}>{action}</Badge>
                        </td>
                        <td className="py-2 text-slate-700 dark:text-slate-200">
                          {typeof scoreVal === 'number' ? Number(scoreVal).toFixed(2) : '—'}
                        </td>
                        <td className="py-2 text-slate-700 dark:text-slate-200">
